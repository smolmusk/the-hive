...
const LIQUID_STAKING_YIELDS_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes
const MAX_POOL_ENTRIES = 5000;

let cachedLiquidStakingYields: {
  timestamp: number;
  pools: LiquidStakingYieldsResultBodyType;
} | null = null;
let inflightLiquidStakingYields: Promise<LiquidStakingYieldsResultBodyType> | null = null;
...
const fetchLiquidStakingPools = async (): Promise<LiquidStakingYieldsResultBodyType> => {
  if (inflightLiquidStakingYields) return inflightLiquidStakingYields;

  inflightLiquidStakingYields = (async () => {
    ...
    try {
      ...
      if (solLiquidStakingPools.length === 0) {
        throw new Error(
          'No Solana liquid staking pools found for the target protocols (Jito, Marinade, Drift, Binance, Bybit, Helius, Jupiter, BlazeStake, Sanctum, Lido).',
        );
      }
      ...
      recordTiming('tool.liquid-staking-yields.upstream', durationMs);
      inflightLiquidStakingYields = null;
    }
  })();

  return inflightLiquidStakingYields;
};
...
export async function getLiquidStakingYields(
  ...
  try {
    const isCacheFresh =
      cachedLiquidStakingYields &&
      Date.now() - cachedLiquidStakingYields.timestamp < LIQUID_STAKING_YIELDS_CACHE_TTL_MS;

    const body =
      isCacheFresh && cachedLiquidStakingYields
        ? cachedLiquidStakingYields.pools
        : await fetchLiquidStakingPools();
    ...
    if (limitedBody && limitedBody.length === 3) {
      ...
      const sortedByScore = limitedBody
        .slice()
        .sort((a, b) => (scorer ? scorer(b) - scorer(a) : (b.yield || 0) - (a.yield || 0)));
    ...
    if (message.startsWith('No Solana liquid staking pools found')) {
      return {
    ...
    return {
      message: `Error getting best liquid staking yields: ${message}`,
    };
  }
}
